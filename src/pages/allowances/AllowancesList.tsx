...
export function AllowancesList() {
  ...
  const columns: DataTableColumn<Allowance>[] = [
    { key: 'code', header: t('allowances.code'), dataIndex: 'code' },
    { key: 'name', header: t('allowances.name'), dataIndex: 'name' },
    { key: 'default_amount', header: t('allowances.defaultAmount'), dataIndex: 'default_amount' },
    {
      key: 'taxable',
    ...
    },
    {
      ...
      render: (record) => (
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={(e) => { e.stopPropagation(); show('allowances', record.id); }}>
            <EyeIcon className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={(e) => { e.stopPropagation(); edit('allowances', record.id); }}>
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Button onClick={() => create('allowances')} className="gap-2">
