...
class PayrollService {
  ...
  async approve(id: number): Promise<ApiResponse<Payroll>> {
    const response = await api.patch(ENDPOINTS.payrolls.approve(id));
    return response.data;
  }

  async lock(id: number): Promise<ApiResponse<Payroll>> {
    const response = await api.patch(ENDPOINTS.payrolls.lock(id));
    return response.data;
  }

  async markPaid(id: number): Promise<ApiResponse<Payroll>> {
    const response = await api.patch(ENDPOINTS.payrolls.markPaid(id));
    return response.data;
  }
